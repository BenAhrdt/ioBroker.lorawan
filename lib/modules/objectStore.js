    ...
    async initBridgeObject(id, DiscoveryObject) {
        ...
        try {
            if (DiscoveryObject.payload) {
                ...
                /*
                // Get the role of id
                const currentRole = await this.adapter.getObjectAsync(id);
                this.bridge.currentIds[id].role = currentRole.val;
                this.bridge.devices[deviceId].detectedRoles ??= {};
                this.bridge.devices[deviceId].detectedRoles[currentRole.val] ??= {};
                this.bridge.devices[deviceId].detectedRoles[currentRole.val][id] ??= true; */
            }
        } catch (error) {
            this.adapter.log.error(`error at ${activeFunction}:  ${error} - id: ${id}`);
        }
