    ...
    async initBridgeObject(id, DiscoveryObject) {
        ...
        try {
            if (DiscoveryObject.payload) {
                ...
                this.bridge.devices[deviceId][id] ??= {};
                this.bridge.currentIds[id] = this.bridge.devices[deviceId][id];

                // Assign Entity Type
                const topicparts = DiscoveryObject.topic.split('/');
                const entityType = topicparts[1];
                this.bridge.devices[deviceId].entityType = {};
                this.bridge.devices[deviceId].entityType[entityType] =
                    (this.bridge.devices[deviceId].entityType[entityType] ?? 0) + 1;

                this.bridge.devices[deviceId].discovery ??= [];
                this.bridge.devices[deviceId].discovery.push(DiscoveryObject);
                this.bridge.devices[deviceId].informations ??= {};
                this.bridge.devices[deviceId].informations.lastDiscover = this.bridge.devices[
                    deviceId
                ].discovery.reduce((a, b) => (b.lastDiscover.ts > a.lastDiscover.ts ? b : a)).lastDiscover.ts;

                // Get the role of id
                if (await this.adapter.foreignObjectExists(id)) {
                    const currentObject = await this.adapter.getForeignObjectAsync(id);
                    this.bridge.devices[deviceId][id].role = currentObject.common.role;
                    this.bridge.devices[deviceId].detectedRoles ??= {};
                    this.bridge.devices[deviceId].detectedRoles[currentObject.common.role] ??= [];
                    if (!this.bridge.devices[deviceId].detectedRoles[currentObject.common.role].includes(id)) {
                        this.bridge.devices[deviceId].detectedRoles[currentObject.common.role].push(id);
                    }
                    // Role Assign
                    if (this.roleAssignToDeviceInformations[currentObject.common.role]) {
                        const currentState = await this.adapter.getForeignStateAsync(id);
                        this.bridge.devices[deviceId].informations ??= {};
                        this.bridge.devices[deviceId].informations[
                            this.roleAssignToDeviceInformations[currentObject.common.role].name
                        ] = {};
                        this.bridge.devices[deviceId].informations[
                            this.roleAssignToDeviceInformations[currentObject.common.role].name
                        ].state = currentState;
