...
class objectStoreClass {
    ...
    /**
     *
     * @param cardRoles Roles for displaying in dM cards
     */
    async addCardRoles(cardRoles) {
        try {
            if (cardRoles.lorawan) {
                for (const [key, value] of Object.entries(cardRoles.lorawan)) {
                    for (const item of value) {
                        item.cardinfo = true;
                    }
                    if (!this.lorawan.roleAssignToDeviceInformations[key]) {
                        this.lorawan.roleAssignToDeviceInformations[key] = [];
                    }
                    if (Array.isArray(value)) {
                        this.lorawan.roleAssignToDeviceInformations[key].push(...value);
                    } else {
                        this.lorawan.roleAssignToDeviceInformations[key].push(value);
                    }
                }
            ...
            if (cardRoles.bridge) {
                for (const [key, value] of Object.entries(cardRoles.bridge)) {
                    for (const item of value) {
                        item.cardinfo = true;
                    }
                    if (!this.bridge.roleAssignToDeviceInformations[key]) {
                        this.bridge.roleAssignToDeviceInformations[key] = [];
                    }
                    if (Array.isArray(value)) {
                        this.bridge.roleAssignToDeviceInformations[key].push(...value);
                    } else {
                        this.bridge.roleAssignToDeviceInformations[key].push(value);
                    }
                }
            ...
            if (cardRoles.toIob) {
                for (const [key, value] of Object.entries(cardRoles.toIob)) {
                    for (const item of value) {
                        item.cardinfo = true;
                    }
                    if (!this.toIob.roleAssignToDeviceInformations[key]) {
                        this.toIob.roleAssignToDeviceInformations[key] = [];
                    }
                    if (Array.isArray(value)) {
                        this.toIob.roleAssignToDeviceInformations[key].push(...value);
                    } else {
                        this.toIob.roleAssignToDeviceInformations[key].push(value);
                    }
                }
            ...
            this.adapter.log.error(`error in addCardRoles: ${error}`);
        }
    }

    /**
     *
     * @param message Message to generate the DeviceIds
     */
    setBridgeDeviceinfos(message) {
        this.bridge.receivedDeviceInfos = message.devices;
    }
    ...
    async generateStoreObjects() {
        ...
        try {
            // Get the Bride receivedDeviceInfos if present
            if (this.adapter.objectExists('bridge.devices.receivedDeviceInfos')) {
                const deviceInfos = await this.adapter.getState('bridge.devices.receivedDeviceInfos');
                if (deviceInfos) {
                    this.setBridgeDeviceinfos({ devices: JSON.parse(deviceInfos.val) });
                }
            }
            // Get the States
            ...
            for (const adapterObject of Object.values(adapterObjects)) {
                if (
                    ...
                    if (this.adapter.config.BridgeDevicesToGenerate.includes('toIobDevices')) {
            ...
            for (let i = 0; i < parts.length; i++) {
                ...
                if (isLast) {
                    if (payload !== undefined) {
                        ...
                        if (node[key].object?.type === 'state' || node[key].state) {
    ...
    async updateLoraWanObject(id, options = {}) {
        ...
        try {
            ...
            if (!idObject[id] || !idObject[id].object) {
    ...
    async initBridgeObjectWithDiscover(id, DiscoveryObject) {
        const activeFunction = 'objectStore.js - initBridgeObjectWithDiscover';
        this.adapter.log.silly(`Function ${activeFunction} started.`);
        try {
            ...
            if (DiscoveryObject.payload) {
                ...
                for (const [key, id] of Object.entries(DiscoveryObject.ids)) {
                    ...
                    if (await this.adapter.foreignObjectExists(id)) {
                        ...
                        //deviceObject[deviceId].ids[id] ??= {}; Changed on 31.03.2026
                        //idObject[id] = deviceObject[deviceId].ids[id];
                        idObject[id] ??= {};
                        deviceObject[deviceId].ids[id] = idObject[id];
     ...
     * @param id id, for wich the structure is to build
     */
    async deleteBridgeObject(id) {
        const activeFunction = 'objectStore.js - deleteBridgeObject';
        this.adapter.log.silly(`Function ${activeFunction} started.`);
        let mark = 0;
        try {
            // Get global values
            ...
            // Check for object exists. No => return
            if (!idObject[id]) {
                return;
            }
            mark = 1;
            const deviceId = idObject[id].deviceId;
            delete idObject[id];
            delete deviceObject[deviceId].ids[id];
            if (Object.keys(deviceObject[deviceId].ids).length === 0) {
                delete deviceObject[deviceId];
            } else {
                if (deviceObject[deviceId].PublishedIds[id]) {
                    if (deviceObject[deviceId].PublishedIds[id].discovery[0].state_topic) {
                        delete deviceObject[deviceId].SubscribedTopics[
                            deviceObject[deviceId].PublishedIds[id].discovery[0].state_topic
                        ];
                    }
                    delete deviceObject[deviceId].PublishedIds[id];
                }
                // Iterate to the detected Roles and count down / delete
                const roles = deviceObject[deviceId].detectedRoles;

                for (const role in roles) {
                    mark = 5;
                    if (Object.hasOwn(roles[role], id)) {
                        mark = 6;
            ...
            this.adapter.log.error(`error at ${activeFunction}:  ${error} - id: ${id} - Mark: ${mark}`);
        }
    }
    ...
    applyRules(rules, idSubfolder, key, deviceObject, deviceId, idObject, id) {
        ...
        deviceObject[deviceId].informations ??= {};
        if (rule.cardinfo) {
            deviceObject[deviceId].informations.card ??= {};
            deviceObject[deviceId].informations.card[id] ??= idObject[id];
            deviceObject[deviceId].informations.card[id].rule = rule;
        } else {
    ...
    async initToIobObject(id, options = {}) {
        const activeFunction = 'objectStore.js - initToIobObject';
        this.adapter.log.silly(`Function ${activeFunction} started.`);
        try {
            ...
            const parts = id.split('.').slice(strip);
            if (parts.length === 1 && (!payload || !payload.object || payload?.object?.type === 'state')) {
                return;
            }
            ...
            // Query for init object
            if (!payload?.object || !payload.object.common) {
                return;
            }
            for (let i = 0; i < parts.length; i++) {
                const key = parts[i];
     ...
     * @param options eg. payload wich is set to last element in id
     */
    async updateToIobObject(id, options = {}) {
        const activeFunction = 'objectStore.js - updateToIobObject';
        this.adapter.log.silly(`Function ${activeFunction} started.`);
        let mark = 0;
        try {
            let { strip = 4, payload } = options;
            ...
            // Check for object exists. No => init
            if (!idObject[id] || !idObject[id].object) {
                await this.initToIobObject(id, options);
                return;
            }
            mark = 1;
            const parts = id.split('.').slice(strip);
            ...
            const idSubfolder = parts[1];
            mark = 2;
            // Assign ObjectBefore Object
            const objectBefore = structuredClone(idObject[id]);
            if (payload !== undefined) {
                // Query for null (id deleted)
                mark = 4;
                // Assign payload entries
                for (const [name, value] of Object.entries(payload)) {
                    idObject[id][name] = value;
                ...
                if (idObject[id].object?.type === 'state') {
                    ...
                    if (!idObject[id].state) {
                        if (idObject[id].object._id) {
                            mark = 12;
                            const state = await this.adapter.getForeignStateAsync(idObject[id].object._id);
                            idObject[id].state = state;
                        }
                    }
                    // Assign to device
     ...
     * @param options eg. payload wich is set to last element in id
     */
    async deleteToIobObject(id, options = {}) {
        const activeFunction = 'objectStore.js - deleteToIobObject';
        this.adapter.log.silly(`Function ${activeFunction} started.`);
        let mark = 0;
        try {
            let { strip = 4 } = options;
            ...
            mark = 1;
            const parts = id.split('.').slice(strip);
            // Case whole device will be deleted
            if (parts.length === 1) {
                mark = 2;
                const deviceId = parts[0];
                delete idObject[id];
            ...
            // Case the type / or indicate state will be deleted
            if (parts.length == 2) {
                mark = 3;
                const deviceId = parts[0];
                const type = parts[1];
                delete idObject[id];
                delete deviceObject[deviceId]?.[type];
            }

            // Case the id key will be deleted
            if (parts.length == 3) {
                mark = 4;
                const deviceId = parts[0];
                const type = parts[1];
                const key = parts[2];
                delete idObject[id];
                delete deviceObject[deviceId]?.[type]?.[key];
                ...
                for (const role in roles) {
                    mark = 5;
                    if (Object.hasOwn(roles[role], type)) {
                        mark = 6;
                        roles[role][type] -= 7;
                        if (roles[role][type] === 0) {
                            mark = 8;
                            delete roles[role][type];
