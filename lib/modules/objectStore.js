...
class objectStoreClass {
    ...
    constructor(adapter) {
        ...
        this.assignToDevice = {
            rssi: true,
            devicetype: true,
            json: { name: 'lastUplink', ending: '.uplink.raw.json' },
        };
    }
    ...
    async generateDeviceObjects() {
        ...
        try {
            // Get the States
            const adapterObjects = await this.adapter.getAdapterObjectsCached();
                        ...
                        if (node[key].object && node[key].object.type === 'state') {
                            ...
                            if (Object.hasOwn(this.assignToDevice, key) || assign) {
                                ...
                                if (assign) {
                                    ...
                                    if (
                                        this.assignToDevice[key].ending &&
                                        !id.endsWith(this.assignToDevice[key].ending)
                                    ) {
                                        continue;
                                    }
