        ...
        try {
            if (DiscoveryObject.payload) {
                ...
                if (await this.adapter.foreignObjectExists(id)) {
                    const currentRole = await this.adapter.getForeignObjectAsync(id);
                    this.bridge.devices[deviceId][id].role = currentRole.common.role;
                    this.bridge.devices[deviceId].detectedRoles ??= {};
                    this.bridge.devices[deviceId].detectedRoles[currentRole.common.role] ??= [];
                    if (!this.bridge.devices[deviceId].detectedRoles[currentRole.common.role].includes(id)) {
                        this.bridge.devices[deviceId].detectedRoles[currentRole.common.role].push(id);
                    }
                }
            }
        } catch (error) {
