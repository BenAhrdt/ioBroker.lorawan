...
class LoRaWANDeviceManagement extends DeviceManagement {
     ...
     */
    async listDevices() {
        if (this.adapter.objectStore) {
            this.adapter.log.error('objectStore erkannt');
        }
        if (this.adapter.objectStore.devices) {
            this.adapter.log.error('devices erkannt');
        }
        const arrDevices = [];
        this.adapter.log.warn(JSON.stringify(this.adapter.objectStore.devices));
        for (const [key, value] of Object.entries(this.adapter.objectStore.devices)) {
            const res = {
                id: key,
                name: value.object.common.name,
                icon: `/adapter/${this.adapter.name}/icons/Node.png`, //${value.object.common.icon}`,
