...
class mqttClientClass {
    ...
    constructor(adapter, settings) {
        ...
        this.basefolder = 'info';
        this.adapter.extendObject(`${this.basefolder}.lnsBaseIp`, {
            type: 'state',
            common: {
                name: 'Ip from Home LNS',
                type: 'string',
                role: 'url.blank',
                read: true,
                write: true,
        ...
        this.client.on('error', err => {
            if (this.errorCountdown === 0) {
                this.adapter.log.error(`mqttClientClass error: ${err} check mqtt connection settings`);
                this.errorCountdown = this.numberOfErrorsToLog;
            } else {
                this.errorCountdown--;
            }
    ...
    async publish(topic, message, opt) {
        this.adapter.log[this.adapter.logtypes.mqtt]?.(`Publishing topic: ${topic} with message: ${message}.`);
