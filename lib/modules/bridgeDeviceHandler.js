            ...
            if (message.entities) {
                ...
                for (const entity of Object.values(message.entities)) {
                    ...
                    for (const state of entityInfo.state) {
                        ...
                        if (this.adapter.objectExists(state.id) && stateValue != null && stateValue !== 0) {
        ...
        try {
            ...
            if (entity.domain === 'light') {
                ...
                if (
                    ...
                    entity.capabilities.supported_color_modes.includes('rgb_color')
                ) {
                    const colorObject = {
                        r: 0,
                        g: 0,
    ...
    getKeyOfState(state, states) {
        ...
        for (const [key, value] of Object.entries(states)) {
            if (value === state) {
                ...
                this.adapter.log.warn(`return the entrie: ${commandNumber}`);
                return commandNumber;
            }
        }
        this.adapter.log.warn(`return the state: ${state}`);
        return state;
    }
