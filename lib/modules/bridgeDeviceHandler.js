...
class bridgeDeviceHandlerClass {
        ...
        try {
            ...
            if (entity.domain === 'light') {
                ...
                if (entity.attributes.rgb_color) {
                    hexColor = this.adapter.bridge.rgbToHex({
                        r: entity.attributes.rgb_color[0],
                        g: entity.attributes.rgb_color[1],
                        b: entity.attributes.rgb_color[2],
                    });
                } else if (entity.attributes.hs_color) {
                    hexColor = this.adapter.bridge.hsToHex(
                        entity.attributes.hs_color[0],
                        entity.attributes.hs_color[1],
                    );
                }
                if (hexColor !== null) {
                    state.push({
    ...
    detectRole(entity, type) {
        ...
        try {
            ...
            if (entity.device_class === 'enum' && entity.device?.model?.includes('air quality')) {
            ...
            if (idObject.native.state.isAttribute) {
                ...
                if (idObject.native.entity.domain === 'light') {
                    ...
                    if (id.endsWith('brightness')) {
                        ...
                        const supportedModes = idObject.native.entity.capabilities.supported_color_modes || [];

                        if (supportedModes.includes('rgb')) {
                            sendInfo.attribute = 'rgb_color';

                            const rgb = this.adapter.bridge.hexToRgb(state.val);

                            sendInfo.value = [rgb.r, rgb.g, rgb.b];
                        } else if (supportedModes.includes('hs')) {
                            sendInfo.attribute = 'hs_color';

                            const hs = this.adapter.bridge.hexToHs(state.val);

                            sendInfo.value = [hs.h, hs.s];
                        }
