...
class bridgeDeviceHandlerClass {
     ...
     */
    async setDeviceIds(message) {
        const activeFunction = 'bridgeDeviceHandler.js - setDeviceIds';
        this.adapter.log.silly(`Function ${activeFunction} started.`);
        try {
            if (message.devices) {
                for (const [key, value] of Object.entries(message.devices)) {
                    if (this.adapter.objectStore.bridge.devices[key]) {
                        this.adapter.objectStore.bridge.devices[key].deviceId = value.deviceId;
                    }
                }
            }
        } catch (error) {
