            ...
            if (message.entities) {
                ...
                for (const entity of Object.values(message.entities)) {
                    ...
                    if (message.discovery) {
                        if (!available[entityInfo.device.id]) {
                            available[entityInfo.device.id] =
                                (entity.domain !== 'buton' && entity.domain !== 'event' && entity.domain !== 'scene') ||
                                entity.available;
                            await this.adapter.setState(availableId, available[entityInfo.device.id], true);
                        } else if (available[entityInfo.device.id] === true) {
                            if (
                                entity.domain !== 'buton' &&
                                entity.domain !== 'event' &&
                                entity.domain !== 'scene' &&
