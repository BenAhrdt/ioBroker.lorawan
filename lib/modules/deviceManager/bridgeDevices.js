            ...
            if (deviceValue.entityType.light) {
                ...
                for (const discovery of deviceValue.discovery) {
                    if (discovery.topic.startsWith('homeassistant/light/')) {
                        onOfId = discovery.ids.onOff;
                        break;
                    }
                }
                // Custom information panel rendered as a JSON Config schema.
                // `customInfo` lets you embed arbitrary UI elements in the device row.
                // Here we display the online state as a switch widget using the JSON Config `state` type.
                // JSON Config documentation: https://github.com/ioBroker/ioBroker.admin/blob/master/packages/jsonConfig/SCHEMA.md
                res.customInfo = {
                    id: deviceId,
                    schema: {
                        type: 'panel',
