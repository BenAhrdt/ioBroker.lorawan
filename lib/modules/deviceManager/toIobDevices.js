...
class toIobDevicesClass {
    ...
    async getDeviceDetails(id) {
        ...
        const lastDiscover = new Date(
            this.adapter.objectStore.toIob.devices[id].informations.lastDiscover,
        ...
        const sortedDiscovery = [...this.adapter.objectStore.toIob.devices[id].discovery].sort(
            (a, b) => b.lastDiscover.ts - a.lastDiscover.ts,
        );
        data.Discoveries = JSON.stringify(sortedDiscovery, null, 2);

        if (this.adapter.objectStore.toIob.devices[id].informations.lastPublish) {
            const lastPublish = new Date(
                this.adapter.objectStore.toIob.devices[id].informations.lastPublish,
            ).toLocaleString('de-DE', {
                weekday: 'long', // Mo
                year: 'numeric', // 2026
                month: '2-digit', // 01
        ...
        data.Published = JSON.stringify(this.adapter.objectStore.toIob.devices[id].PublishedIds, null, 2);

        if (this.adapter.objectStore.toIob.devices[id].informations.lastSubscribe) {
            const lastSubscribe = new Date(
                this.adapter.objectStore.toIob.devices[id].informations.lastSubscribe,
            ).toLocaleString('de-DE', {
                weekday: 'long', // Mo
                year: 'numeric', // 2026
                month: '2-digit', // 01
        ...
        data.Subscribed = JSON.stringify(this.adapter.objectStore.toIob.devices[id].SubscribedTopics, null, 2);
        ...
        for (const key in this.extractStateValues(this.adapter.objectStore.toIob.devices[id].ids)) {
            const commonObject = this.adapter.objectStore.toIob.devices[id].ids[key].object.common;
            const name = this.adapter.objectStore.toIob.devices[id].ids[key].name;
            let longkey = key;
        ...
        data.values = JSON.stringify(this.extractStateValues(this.adapter.objectStore.toIob.devices[id].ids), null, 2);*/
        ...
        if (this.adapter.objectStore.toIob.devices[id].PublishedIds) {
        ...
        if (this.adapter.objectStore.toIob.devices[id].SubscribedTopics) {
