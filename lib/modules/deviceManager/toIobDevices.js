        ...
        for (const [deviceId, deviceValue] of Object.entries(this.adapter.objectStore.toIob.devices)) {
            ...
            if (deviceValue.light) {
                ...
                if (deviceValue.light.on) {
                    ids.onOff = deviceValue.light.on.object._id;
                    usedLightId[ids.onOff] = true;
                    card = { preLabel: '⏻ ', label: 'An / Aus' };
                    card = lodash.merge(card, deviceValue.light.on.object.native?.card);
                    ...
                    if (card.name) {
                        ...
                        label = deviceValue.light.on.object._id.substring(
                            deviceValue.light.on.object._id.lastIndexOf('.') + 1,
                        );
                    }
                ...
                if (deviceValue.light.brightness) {
                    ...
                    card = lodash.merge(card, deviceValue.light.brightness.object.native?.card);
                    const preLabel = card.preLabel ?? '';
                    let label = '';
                    if (card.name) {
                        label = card.name;
                        ...
                        label = deviceValue.light.brightness.object._id.substring(
                            deviceValue.light.brightness.object._id.lastIndexOf('.') + 1,
                        );
                    }
