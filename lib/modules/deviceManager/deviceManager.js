...
class LoRaWANDeviceManagement extends DeviceManagement {
    ...
    async getInstanceInfo() {
        ...
        if (this.adapter.config.origin !== 'off') {
            info.actions.push({
                id: 'cofigureLoWANBaseIp',
                title: `Base Ip for LoraWAN devices: ${(await this.adapter.getStateAsync(`${this.adapter.namespace}.info.lnsBaseIp`)).val}`,
                icon: 'edit',
                description: 'Set the Base Ip for LoRaWAN devices',
                handler: context => this.changeBaseIpLoRaWAN(context),
            });
        }
        if (this.adapter.config.BridgeType !== 'off') {
            info.actions.push({
                id: 'cofigureBridgeBaseIp',
                ...
                handler: context => this.changeBaseIpBridge(context),
            });
        }
        return info;
    }
     ...
     */
    async changeBaseIpLoRaWAN(context) {
        const result = await context.showForm(
            ...
            {
                data: {
                    baseIp: (await this.adapter.getStateAsync(`${this.adapter.namespace}.info.lnsBaseIp`)).val,
                },
                title: this.adapter.i18nTranslation['Change Base Ip for LoRaWAN devices'],
                buttons: ['apply', 'cancel'],
            },
        );
        if (result && typeof result.baseIp === 'string') {
            this.adapter.setState(`${this.adapter.namespace}.info.lnsBaseIp`, result.baseIp, true);
        }
        return { refresh: true };
    }
     ...
     * @param context Context for Instance info
     */
    async changeBaseIpBridge(context) {
        const result = await context.showForm(
            {
                type: 'panel',
                items: {
    ...
    async loadDevices(context) {
        this.adapter.log[this.adapter.logtypes.deviceManager]?.(`loadDevices started`);
        const deviceCount =
