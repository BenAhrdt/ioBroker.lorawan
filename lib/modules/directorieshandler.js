...
class directorieshandlerClass {
    ...
    constructor(adapter) {
        ...
        this.HABridgeAdaptername = 'mqtt-client';
    ...
    async setCustomForHaBridge(objectId, common) {
        ...
        if (this.adapter.config.MQTTInstance.indexOf(this.HABridgeAdaptername) !== -1) {
            ...
            if (objectId.indexOf(`${this.reachableSubfolders.uplinkDecoded}.`) !== -1) {
                if (this.adapter.config.HABridgeDecoded !== 'off') {
                    ...
                    const changeInfo = await this.adapter.getChangeInfo(objectId);
                    const DeviceIdentifier = this.getDeviceIdentifier(changeInfo, this.adapter.config.DeviceIdentifier);
     ...
     * @param DeviceIdentifier configed deviceidentifier
     */
    getDeviceIdentifier(changeInfo, DeviceIdentifier) {
        switch (DeviceIdentifier) {
            case 'deviceEUI':
                return changeInfo.deviceEUI;

            case 'deviceId':
                return changeInfo.deviceId;

            case 'usedDeviceId':
                this.adapter.log.error(changeInfo.usedDeviceId);
                return changeInfo.usedDeviceId;
        }
    }
}
