...
class directorieshandlerClass {
    ...
    async setCustomForHaBridge(objectId, common) {
        if (objectId.indexOf(`${this.reachableSubfolders.uplinkDecoded}.`) !== -1) {
            ...
            if (!common.custom['mqtt-client.0']) {
                common.custom['mqtt-client.0'] = {};
            }
            common.custom['mqtt-client.0'] = {
                enabled: true,
                publish: true,
            ...
            const changeInfo = await this.adapter.getChangeInfo(objectId);
            // this.adapter.log.error(JSON.stringify(changeInfo));
            common.custom['mqtt-client.0'].topic = `iobroker/${changeInfo.usedDeviceId}/${changeInfo.changedState}`;
        }
    }
}
