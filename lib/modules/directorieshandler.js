...
class directorieshandlerClass {
    ...
    constructor(adapter) {
        ...
        this.HABridge = {
            ...
            Dummy: {
                ...
                usedApplicationName: 'FensterStaße',
                deviceEUI: 'a84041a52a598cc5',
                deviceId: 'Balkon',
                usedDeviceId: 'BalkonLinks',
            },
        };
        ...
        //const HaBridgeConfig = this.buildHaBridgeConfig();
    }
                    ...
                    if (
                        ...
                        if (typeof obj[elementName] === 'object' && obj[elementName] && !obj[elementName].isState) {
                            ...
                            if (objectId.indexOf(`${this.reachableSubfolders.uplinkDecoded}.`) !== -1) {
                                ...
                                if (this.adapter.config.customDecodedJsonActive) {
                                    if (typeof this.customObject === 'object') {
                                        // Eventuell noch einen Merge einfügen
                            ...
                            if (objectId.indexOf(`${this.reachableSubfolders.configuration}.`) !== -1) {
                                // Check custom configuration jsondev-server
                                if (
                            ...
                            // Add LoRaWAN 2 HA Bridged Funktion (MQTT Topic used with Script for auto discovery)
                            // Hinzufügen bei neuen States
                            if (!(await this.adapter.objectExists(`${objectId}`))) {
                                await this.setCustomForHaBridge(objectId, common);
                            }
    ...
    /**
     * @param objectId Id of actual element
     * @param common custom object of state
     */
    async setCustomForHaBridge(objectId, common) {
        // Abfrage, ob überhaupt eine Instanz ausgewählt wurde
        if (this.adapter.config.MQTTInstance.indexOf(this.HABridge.Adaptername) !== -1) {
            // Abfrage des Decoded Ordners
            if (objectId.indexOf(`${this.reachableSubfolders.uplinkDecoded}.`) !== -1) {
                if (this.adapter.config.HABridgeDecoded !== 'off') {
                    const changeInfo = await this.adapter.getChangeInfo(objectId);
                    if (!common.custom) {
                        common.custom = {};
                    }
                    if (!common.custom[this.adapter.config.MQTTInstance]) {
                        common.custom[this.adapter.config.MQTTInstance] = {};
                    }
                    const HABridgeCustom = {
                        enabled: true,
                        publish: true,
                    };
                    if (this.adapter.config.HABridgeDecoded === 'custom') {
                        // Custom holen und mergen
                        const Actualobject = await this.adapter.getObjectAsync(objectId);

                        // Abfrage ob das Objekt schon besteht (Zuweisung nicht )
                        // Mergen nur, wenn State bereits besteht
                        if (Actualobject !== null) {
                            if (Actualobject.common.custom) {
                                common.custom = lodash.merge(common.custom, Actualobject.common.custom);
                            }
                        }

                        // HA Bridge
                        HABridgeCustom.enabled = false;
                        HABridgeCustom.publish = false;
                        // Abfrage, ob der State in der config zugewiesen wurde
                        if (this.adapter.config.HABridgeConfig) {
                            for (const config of this.adapter.config.HABridgeConfig) {
                                if (
                                    (changeInfo.applicationId === config.Application || config.Application === '*') &&
                                    (changeInfo.deviceEUI === config.Device || config.Device === '*') &&
                                    (objectId.includes(`.${config.Folder}.`) || config.Folder === '*') &&
                                    (objectId.endsWith(`${this.reachableSubfolders.uplinkDecoded}.${config.State}`) ||
                                        config.State === '*')
                                ) {
                                    HABridgeCustom.enabled = !config.exclude;
                                    HABridgeCustom.publish = config.publish;
                                    if (config.exclude) {
                                        break;
                                    }
                                }
                    ...
                    common.custom[this.adapter.config.MQTTInstance] = {
                        enabled: HABridgeCustom.enabled,
                        publish: HABridgeCustom.publish,
                        retain: true,
                    ...
                    common.custom[this.adapter.config.MQTTInstance].topic =
                        `${this.HABridge.TopicPrefix}${DeviceIdentifier}/${changeInfo.changedState}`;
                }
                // Abfrage des Control Ordners
            } else if (objectId.indexOf(`${this.reachableSubfolders.downlinkControl}.`) !== -1) {
                if (this.adapter.config.HABridgeControl !== 'off') {
                    const changeInfo = await this.adapter.getChangeInfo(objectId);
                    if (!common.custom) {
                        common.custom = {};
                    }
                    ...
                    const HABridgeCustom = {
                        enabled: true,
                        publish: true,
                        subscribe: this.adapter.config.HABridgeControl === 'publishsubscribe',
                    };
                    if (this.adapter.config.HABridgeControl === 'custom') {
                        // Custom holen und mergen
                        const Actualobject = await this.adapter.getObjectAsync(objectId);
                        // Abfrage ob das Objekt schon besteht (Zuweisung nicht )
                        // Mergen nur, wenn State bereits besteht
                        ...
                        HABridgeCustom.enabled = false;
                        HABridgeCustom.publish = false;
                        HABridgeCustom.subscribe = false;
                        // Abfrage, ob der State in der config zugewiesen wurde
                        if (this.adapter.config.HABridgeConfig) {
                            for (const config of this.adapter.config.HABridgeConfig) {
                                if (
                                    ...
                                    (changeInfo.deviceEUI === config.Device || config.Device === '*') &&
                                    (objectId.includes(`.${config.Folder}.`) || config.Folder === '*') &&
                                    (objectId.endsWith(`${this.reachableSubfolders.downlinkControl}.${config.State}`) ||
                                        config.State === '*')
                                ) {
                                    HABridgeCustom.enabled = !config.exclude;
                                    HABridgeCustom.publish = config.publish;
                                    HABridgeCustom.subscribe = config.subscribe;
                                    if (config.exclude) {
                                        break;
                                    }
                                }
                    ...
                    common.custom[this.adapter.config.MQTTInstance] = {
                        ...
                        pubAsObject: false,
                        qos: false,
                        subscribe: HABridgeCustom.subscribe,
                        subChangesOnly: false,
                        subAsObject: false,
                        subQos: false,
                        setAck: false,
    ...
    getDeviceIdentifier(changeInfo, DeviceIdentifiers) {
        ...
        return this.replaceGermanSpecific(DeviceIdentifier);
    }
    ...
    /**
     * Builds the internal Config for the Bridge to Home Assistant
     */
    buildHaBridgeConfig() {
        const Config = {};
        for (const config of this.adapter.config.HABridgeConfig) {
            const Id = `${config.Application}.devices.${config.Device}.${config.Folder}.${config.State}`;
            Config[Id] = config.publish;
            Config[Id] = config.subscribe;
        }
        return Config;
    }
}
