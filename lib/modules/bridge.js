...
class bridgeClass {
    ...
    async getDiscoveryObject(changeInfo, options) {
        ...
        try {
            ...
            const StateNameWithoutDot = this.replaceDot(StateName);
            ...
            const DiscoveryTopic = `${this.BridgeDiscoveryPrefix[this.adapter.config.BridgeType]}${EntityType}/${ReplaceWithoutSlash}/${StateNameWithoutDot}/config`;
            const DiscoveryPayload = {
                name: StateName,
     ...
     * @param Inputstring string to replace the vorbidden chars
     */
    replaceDot(Inputstring) {
        const activeFunction = 'bridge.js - replaceDot';
        this.adapter.log.debug(`Function ${activeFunction} started.`);
        try {
            return Inputstring.replace(/\./g, '_');
        } catch (error) {
