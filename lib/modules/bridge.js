 ...
 */
class bridgeClass extends EventEmiter {
    /**
        ...
        try {
            if (this.SubscribedTopics[topic]) {
                ...
                if (this.SubscribedTopics[topic].light) {
                    ...
                    if (message.color_temp) {
                        await this.adapter.setForeignStateAsync(
                            this.SubscribedTopics[topic].LightIds.colorTemperature,
                            {
                                val: message.color_temp,
                                c: 'from bridge',
                            },
                        );
                    }
                ...
                if (this.SubscribedTopics[topic].id.startsWith(this.adapter.namespace)) {
                    ...
                    if (this.SubscribedTopics[topic].dataExchange) {
                        if (typeof message === 'object') {
                            ...
                            if (this.bridgeDeviceHandler.checkVersion(message)) {
                                // Check message type
                                if (message.deviceIds) {
                                    await this.bridgeDeviceHandler.setDeviceinfos(message);
                                } else {
    ...
    /**
     * Converts a HEX color string to HS values.
     *
     * @param {string} hex - HEX color string (#RRGGBB)
     * @returns {{h: number, s: number}} HS color object
     */
    hexToHs(hex) {
        const activeFunction = 'bridge.js - hexToHs';
        this.adapter.log.silly(`Function ${activeFunction} started.`);
        try {
    ...
    /**
     * Converts a HS color value to a HEX string.
     *
     ...
     */
    hsToHex(h, s) {
        const activeFunction = 'bridge.js - hsToHex';
        this.adapter.log.silly(`Function ${activeFunction} started.`);
        try {
            s /= 100;
    ...
    async discovery(id, options) {
        ...
        try {
            if (!this.CheckedIds[id] || options?.forceDiscovery) {
                this.CheckedIds[id] = {};
                // Check for logging
                this.adapter.log[this.adapter.logtypes.bridgeDiscovery]?.(`discover the id ${id}`);
                return await this.buildDiscovery(id, options);
            }
            // Check for logging
            this.adapter.log[this.adapter.logtypes.bridgeDiscovery]?.(`${id} allready checked for discovery`);
            return false;
        } catch (error) {
            this.adapter.log.error(`error at ${activeFunction}:  ${error}`);
        }
    ...
    async discoverClimate() {
        ...
        try {
            if (this.adapter.config.ClimateConfig) {
                if (this.adapter.config.ClimateConfig.length != 0) {
                    this.adapter.log.debug(
                        `${activeFunction} called with config: ${JSON.stringify(this.adapter.config.ClimateConfig)}`,
                    );
                }
                for (const config of this.adapter.config.ClimateConfig) {
                    ...
                    const Id = climateUniqueString?.flat + this.EndingVirtualClimate; // geändert 12.01.2026 config.climateIds.target.substring(0, indexLastDotTarget) + this.EndingVirtualClimate;
    ...
    async publishId(id, val, options) {
        ...
        try {
            if (this.PublishedIds[id]) {
                // First emit the Publish
                this.emit('publishId', { device: this.PublishedIds[id].discovery[0].payload.device });
                if (val === undefined) {
                ...
                if (val === null || val === undefined) {
                    this.adapter.log.warn(`No value at id: ${id}. Publish will not executed`);
                    return;
                }
        ...
        try {
            ...
            if (id.includes(`${this.adapter.messagehandler.directoryhandler.reachableSubfolders.uplinkDecoded}.`)) {
                ...
                if (this.adapter.config.BridgeStateConfig) {
                    let idKey = changeInfo.changedState;
    ...
    assignTopicStructure(assignObject, indexTopic, informations, topic, payload, id, options) {
        ...
        assignObject[indexTopic].id = id;
        assignObject[indexTopic].informations = structuredClone(informations);
        if (options) {
            for (const option in options) {
    ...
    async publishDiscovery(id, DiscoveryObject) {
        ...
        try {
            ...
            this.DiscoveredUniqueId[DiscoveryObject.payload.unique_id] = {};
    ...
    async discoverDataExchange() {
        ...
        options.common = deviceObject.common;
        changeInfo = { id: `bridge.dataToIob` }; // 26.01.2026 to unse getChangeInfo with objectstore
        options.Bridgestate = {
            discover: true,
            publish: false,
            subscribe: true,
    ...
    async checkDiscoveries() {
        ...
        try {
            for (const id in this.OldDiscoveredIds) {
                if (!this.DiscoveredIds[id]) {
                    // Check unique id id not set (in the past)
                    if (!this.DiscoveredUniqueId[this.OldDiscoveredIds[id].payload.unique_id]) {
    ...
    async discoverForeignClimate(config) {
        ...
        try {
            ...
            this.adapter.setTimeout(async () => {
                ...
                // await this.adapter.subscribeForeignObjectsAsync(config.climateIds.target);
                await this.adapter.subscribeForeignStatesAsync(config.climateIds.act);
                // await this.adapter.subscribeForeignObjectsAsync(config.climateIds.act);
                await this.adapter.subscribeForeignStatesAsync(config.climateIds.mode);
                // await this.adapter.subscribeForeignObjectsAsync(config.climateIds.mode);
            }, 1000);
        } catch (error) {
            this.adapter.log.error(`error at ${activeFunction}:  ${error}`);
        }
    ...
    async discoverForeignHumidifier(config) {
        ...
        try {
            ...
            const Id = humidifierUniqueString?.flat + this.EndingVirtualHumidifier; // geändert 12.01.2026 config.HumidifierIds.onOff.substring(0, indexLastDotOnOff) + this.EndingVirtualHumidifier;
            ...
            this.adapter.setTimeout(async () => {
                ...
                // await this.adapter.subscribeForeignObjectsAsync(config.HumidifierIds.onOff);
                await this.adapter.subscribeForeignStatesAsync(config.HumidifierIds.target);
                // await this.adapter.subscribeForeignObjectsAsync(config.HumidifierIds.target);
                if (config.WithAct) {
                    await this.adapter.subscribeForeignStatesAsync(config.HumidifierIds.act);
                    // await this.adapter.subscribeForeignObjectsAsync(config.HumidifierIds.act);
                }
            }, 1000);
        } catch (error) {
            this.adapter.log.error(`error at ${activeFunction}:  ${error}`);
    ...
    async generateForeignHumidifierIds(config) {
        ...
        this.adapter.log.silly(`Function ${activeFunction} started.`);
        try {
            const HumidifierIds = { onOff: '', target: '' };
    ...
    async discoverForeignLight(config) {
        ...
        try {
            ...
            const Id = lightUniqueString?.flat + this.EndingVirtualLight; // geändert 12.01.2026 config.LightIds.onOff.substring(0, indexLastDotOnOff) + this.EndingVirtualLight;
            ...
            if (config.LightColorTemperature) {
                if (!DiscoveryPayload.supported_color_modes) {
                    DiscoveryPayload.supported_color_modes = [];
                }
                DiscoveryPayload.supported_color_modes.push('color_temp');
                const colorTemperatureObject = await this.adapter.getForeignObjectAsync(
                    config.LightIds.colorTemperature,
                );
                if (colorTemperatureObject.common.unit && colorTemperatureObject.common.unit === 'mired') {
                    if (colorTemperatureObject.common.min) {
                        DiscoveryPayload.min_mireds = colorTemperatureObject.common.min;
                    }
                    if (colorTemperatureObject.common.max) {
                        DiscoveryPayload.max_mireds = colorTemperatureObject.common.max;
                    }
                } else {
                    DiscoveryPayload.color_temp_kelvin = true;
                    if (colorTemperatureObject.common.min) {
                        DiscoveryPayload.min_kelvin = colorTemperatureObject.common.min;
                    }
                    if (colorTemperatureObject.common.max) {
                        DiscoveryPayload.max_kelvin = colorTemperatureObject.common.max;
                    }
                }
            }
            const effectSet = {};
            ...
            this.adapter.setTimeout(async () => {
                ...
                if (config.LightColorTemperature) {
                    await this.publishId(config.LightIds.colorTemperature, undefined, {});
                }
                if (config.LightEffects) {
                ...
                // await this.adapter.subscribeForeignObjectsAsync(config.LightIds.onOff);
                if (config.LightBrightness) {
                    await this.adapter.subscribeForeignStatesAsync(config.LightIds.brightness);
                    // await this.adapter.subscribeForeignObjectsAsync(config.LightIds.brightness);
                }
                if (config.LightColor) {
                    await this.adapter.subscribeForeignStatesAsync(config.LightIds.color);
                    // await this.adapter.subscribeForeignObjectsAsync(config.LightIds.color);
                }
                if (config.LightColorTemperature) {
                    await this.adapter.subscribeForeignStatesAsync(config.LightIds.colorTemperature);
                    // await this.adapter.subscribeForeignObjectsAsync(config.LightIds.colorTemperature);
                }
                if (config.LightEffects) {
                    await this.adapter.subscribeForeignStatesAsync(config.LightIds.effects);
                    // await this.adapter.subscribeForeignObjectsAsync(config.LightIds.effects);
                }
            }, 1000);
        } catch (error) {
            this.adapter.log.error(`error at ${activeFunction}:  ${error}`);
    ...
    async generateForeignLightIds(config) {
        ...
        try {
            ...
            if (config.LightColorTemperature) {
                LightIds.colorTemperature = config.ColorTemperatureId;
            }
            if (config.LightEffects) {
    ...
    async discoverForeignCover(config) {
        ...
        try {
            ...
            if (config.CoverIds.number) {
                ...
                if (numberObject.common.states) {
                    for (const [key, value] of Object.entries(numberObject.common.states)) {
                        setAssign[value.toUpperCase()] = { id: config.CoverIds.number, val: Number(key) };
                    }
                } else {
                    ...
                    setAssign.PATRITAL_OPEN = { id: config.CoverIds.number, val: 4 };
                }
            ...
            // Number Signal
            if (config.CoverIds.numberSignal) {
                Cover.numberSignal = {};
                Cover.numberSignal.DeviceIdentifier = (
                    await this.getParentNameing(config.CoverIds.numberSignal)
                )?.parentName;
            }
            ...
            const Id = coverUniqueString?.flat + this.EndingVirtualCover; // geändert 12.01.2026 `${generalId}.${config.CoverName}${this.EndingVirtualCover}`;
            //const Id = `xxx.${config.CoverName}${this.EndingVirtualCover}`;
            ...
            if (config.CoverIds.numberSignal) {
                Cover.numberswitch = {};
                const numberSignalAssign = {};
                const numberSwitchObject = await this.adapter.getForeignObjectAsync(config.CoverIds.numberSignal);
                if (numberSwitchObject.common.states) {
                    for (const [key, value] of Object.entries(numberSwitchObject.common.states)) {
                        numberSignalAssign[Number(key)] = value.toLowerCase();
                    }
                } else {
                    numberSignalAssign[0] = 'closed';
                    numberSignalAssign[1] = 'open';
                    numberSignalAssign[2] = 'ventilation_position';
                    numberSignalAssign[3] = 'position_unknown';
                }
                this.assignIdStructure(
                    this.PublishedIds,
            ...
            this.adapter.setTimeout(async () => {
                ...
                if (config.CoverIds.position) {
                    ...
                    // await this.adapter.subscribeForeignObjectsAsync(config.CoverIds.position);
                }
                if (config.CoverIds.simulatePosition) {
                ...
                if (config.CoverIds.tilt) {
                    ...
                    // await this.adapter.subscribeForeignObjectsAsync(config.CoverIds.tilt);
                }
                if (config.CoverIds.numberSignal) {
                    await this.publishId(config.CoverIds.numberSignal, undefined, {});
                    await this.adapter.subscribeForeignStatesAsync(config.CoverIds.numberSignal);
                    // await this.adapter.subscribeForeignObjectsAsync(config.CoverIds.numberSignal);
                }
                if (config.CoverIds.openSignal) {
                    ...
                    // await this.adapter.subscribeForeignObjectsAsync(config.CoverIds.openSignal);
                }
                if (config.CoverIds.closedSignal) {
                    ...
                    // await this.adapter.subscribeForeignObjectsAsync(config.CoverIds.closedSignal);
                }
            }, 1000);
        } catch (error) {
            this.adapter.log.error(`error at ${activeFunction}:  ${error}`);
    ...
    async generateForeignCoverIds(config) {
        ...
        try {
            ...
            if (config.CoverNumberSignal) {
                CoverIds.numberSignal = config.NumberSignalId;
            } else {
    ...
    async discoverForeignLock(config) {
        ...
        try {
            ...
            const Id = lockUniqueString?.flat + this.EndingVirtualLock; // geändert 12.01.2026 generalId.substring(0, indexLastDotOnOff) + this.EndingVirtualLock;
            ...
            this.adapter.setTimeout(async () => {
                ...
                if (config.LockIds.state) {
                    ...
                    // await this.adapter.subscribeForeignObjectsAsync(config.LockIds.state);
                }
            }, 1000);
        } catch (error) {
            this.adapter.log.error(`error at ${activeFunction}:  ${error}`);
        ...
        try {
            ...
            for (const state of states.rows) {
                if (!clear) {
                    ...
                    await this.adapter.objectStore.deleteBridgeObject(state.id);
    ...
    async getParentNameing(id) {
        ...
        try {
            ...
            if (typeof parentDevice !== 'object') {
                // Check for logging
                this.adapter.log[this.adapter.logtypes.parentNameing]?.(`Parentdevice: ${parentDevice}`);
                ...
                if (typeof parentChannel !== 'object') {
                    ...
                    // Check for logging
                    this.adapter.log[this.adapter.logtypes.parentNameing]?.(
                        `Parentchannel: ${JSON.stringify(parentChannel)}`,
                    );
                ...
                // Check for logging
                this.adapter.log[this.adapter.logtypes.parentNameing]?.(
                    `Parentdevice: ${JSON.stringify(parentDevice)}`,
                );
    ...
    async discoverForeignStandardEntity(id, options) {
        ...
        try {
            ...
            this.adapter.log[this.adapter.logtypes.bridgeDiscovery]?.(
                `Assigned - deviceIdentifier: ${deviceIdentifier} - statename: ${statename}`,
            );
            options.Bridgestate = {
                publish: true,
            ...
            // Subscribe state and Object for On X Change methode
    ...
    async getParentDevice(id) {
        ...
        */
    /**
     * @param {string} id id to get parent Device
     */
    async getParentDevice(id) {
        const activeFunction = 'bridge.js - getParentDevice';
        this.adapter.log.silly(`Function ${activeFunction} started.`);
        ...
        try {
            ...
            // Require at least 2 dots (adapter.instance.device)
            if (parts.length < 3) {
                const lastDot = originalId.lastIndexOf('.');
                return lastDot !== -1 ? originalId.substring(0, lastDot) : originalId;
            }

            let current = id;

            // Walk up the id hierarchy step by step (right to left)
            while (current.includes('.')) {
                current = current.substring(0, current.lastIndexOf('.'));

                // Check local device cache instead of async DB call
                const device = this.adapter.deviceMap.get(current);
                if (device) {
                    return device;
                }
            }

            // No device found → return original id up to last dot
            return originalId.substring(0, originalId.lastIndexOf('.'));
        } catch (error) {
            this.adapter.log.error(`error at ${activeFunction}: ${error}`);

            // Fallback: keep original behavior
            const lastDot = id.lastIndexOf('.');
            return lastDot !== -1 ? id.substring(0, lastDot) : id;
        }
    }
    ...
    */
    ...
    /**
     * @param {string} id id to get parent Channel
     */
    async getParentChannel(id) {
        const activeFunction = 'bridge.js - getParentChannel';
        this.adapter.log.silly(`Function ${activeFunction} started.`);
        ...
        try {
            ...
            // Require at least adapter.instance.device (3 segments)
            if (parts.length < 3) {
                const lastDot = originalId.lastIndexOf('.');
                return lastDot !== -1 ? originalId.substring(0, lastDot) : originalId;
            }
            ...
            while (current.includes('.')) {
                ...
                // Check local channel cache instead of async DB call
                const channel = this.adapter.channelMap.get(current);
                if (channel) {
                    // Channel found → return channel object
                    return channel;
                }
            }

            // No channel found → return original id up to last dot
            return originalId.substring(0, originalId.lastIndexOf('.'));
        } catch (error) {
            this.adapter.log.error(`error at ${activeFunction}: ${error}`);
