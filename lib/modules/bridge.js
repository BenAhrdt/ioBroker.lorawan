...
class bridgeClass {
    ...
    constructor(adapter) {
        ...
        this.GeneralNotificationId = '.general.notification';
    ...
    async discoverGeneralNotification() {
        ...
        this.adapter.log.debug(`Function ${activeFunction} started.`);
        try {
            const device = this.adapter.namespace;
            const normalizedDevice = this.normalizeString(device);
            const notificationId = `${device}${this.GeneralNotificationId}`;
            const discoveryobject = {
                topic: `${this.BridgeDiscoveryPrefix[this.adapter.config.BridgeType]}${this.NotificationEntityType}/${normalizedDevice}/generalNotification/config`.toLocaleLowerCase(),
                payload: {
                    automation_type: 'trigger',
                    topic: `${this.bridgeMqttClient.BridgePrefix}${this.NotificationEntityType}`,
                    type: 'notification',
                    subtype: 'general',
                    device: {
                        identifiers: [normalizedDevice],
                        name: device,
                    },
                },
            };
            ...
            await this.publishDiscovery(notificationId, this.PublishedIds[discoveryobject]);
        } catch (error) {
            this.adapter.log.error(`error at ${activeFunction}:  ${error}`);
        }
    }
