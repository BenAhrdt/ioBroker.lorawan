    ...
    */
    ...
    async discoverForeignCover(config) {
        ...
        try {
                ...
                if (numberObject.common.states) {
                    for (const [key, value] of Object.entries(numberObject.common.states)) {
                        if (value.startsWith === 'OPEN') {
                            setAssign[key] = { id: config.CoverIds.number, val: 'OPEN' };
                            continue;
                        }
                        if (value.startsWith === 'STOP') {
                            setAssign[key] = { id: config.CoverIds.number, val: 'STOP' };
                            continue;
                        }
                        if (value.startsWith === 'CLOSE') {
                            setAssign[key] = { id: config.CoverIds.number, val: 'CLOSE' };
                            continue;
                        }
                    }
                } else {
                    setAssign[1] = { id: config.CoverIds.command, val: 'OPEN' };
                    setAssign[2] = { id: config.CoverIds.command, val: 'CLOSE' };
                    setAssign[3] = { id: config.CoverIds.command, val: 'STOP' };
                }
                Cover.position.DeviceIdentifier = (await this.getParentNameing(config.CoverIds.position))?.parentName;
            }
            ...
            if (config.CoverIds.open || config.CoverIds.command) {
